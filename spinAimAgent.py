"""
*  Spin-Aim agent for playing asteroids
*
*  Authors:
*  Joe Netti
*  Eric Tiano
    ...
    * @param action
    ...
    def act(self, ob, reward, done):
        if self.state.round == 0:  # there are 2 astroid-only frames in a row when game begins
            ...
            if self.state.deadShip:  # if ship is dead, we cannot make a decision
                action = noop
            else:
                action = self.state.findAimDecide(ob, self.spinAim)
#                printAction(action)
            self.state.lastAction = action
        elif self.state.round % 4 == 3:
            action = noop
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env_id', nargs='?', default='AsteroidsNoFrameskip-v4',
                        help='Select the environment to run')  # We choose to use no frameskip rom. The other rom doesn't work
